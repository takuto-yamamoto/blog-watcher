...
- **E2Eテスト (5%)**: アプリ起動とヘルスチェック応答を確認する程度に限定。フルE2Eは高コストかつ脆い。
...
非同期HTTPの統合テストは、基本的に **`respx` を第一選択** とする。

**判定基準:**
- **非同期忠実度**: AsyncClientの実挙動（ストリーミング、並列性、タイムアウト）を再現したいなら `aiohttp`
- **速度/安定性**: 高速・安定性優先なら `respx`
- **対象レイヤ**: ユニット〜軽い統合は `respx`、挙動検証が必要な統合は `aiohttp`
...
| `test_path_expansion_resolves_tilde` | データベースパス `~/data/db.sqlite` | 絶対パスに展開 |
| `test_missing_required_field_raises_error` | `slack.webhook_url` が未設定 | 説明的な検証エラー |

**統合テスト:** なし（純粋な検証ロジック）

**主要エッジケース:**
- 空の設定ファイル、無効なTOML構文、型の不一致
- `~` と環境変数のパス展開
- オプションフィールドのデフォルト値
